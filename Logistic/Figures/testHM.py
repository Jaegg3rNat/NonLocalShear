...
def rho(mu, pe, w, file_path):
    D = 1e-4
    comp_rad = 0.2
    r = mu * D / comp_rad ** 2

    base_folder = f"{file_path}Data"
...
for j in tqdm(range(nx)):
    for i in (range(ny)):
        rho_matrix2[j, i] = rho(mu[j], Pe[i], 0,'../Data/q_0.2')
data2 = rho_matrix2.T
dZdx = np.gradient(rho_matrix2, axis=1)  # Gradient along x
dZdy = np.gradient(rho_matrix2, axis=0)  # Gradient along y

# Calculate the magnitude of the gradient
# data2 = np.sqrt(dZdx ** 2 + dZdy ** 2)

# Normalize the data
data1 = (data1 - data1.min()) / (data1.max() - data1.min())
data2 = (data2 - data2.min()) / (data2.max() - data2.min())

# Create an RGB image: (R, G, B)
heatmap_rgb = np.zeros((*data1.shape, 3))  # Create an empty RGB array
# heatmap_rgb[..., 0] = data1  # Assign to red channel
# heatmap_rgb[..., 2] = data2  # Assign to blue channel

plt.imshow(heatmap_rgb, origin="lower")
plt.colorbar()  # Optional
plt.title("Combined Heatmap (Red: Data1, Blue: Data2)")
plt.show()

fig, ax = plt.subplots()
hm1 = ax.imshow(data1, cmap='Reds', alpha=0.7, origin="lower")  # Red colormap with transparency
hm2 = ax.imshow(data2, cmap='Blues', alpha=0.7, origin="lower") # Blue colormap with transparency

plt.colorbar(hm1, ax=ax, fraction=0.046, pad=0.04)
