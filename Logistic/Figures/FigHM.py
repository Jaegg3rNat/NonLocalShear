...
def create_custom_colormap_with_white_band(vmin, vmax, white_start=1.0, white_end=1.02, n_colors=256):
    assert vmin < white_start < white_end < vmax, "white range must be within data range"

    # Compute relative positions
    rel_white_start = (white_start - vmin) / (vmax - vmin)
    rel_white_end = (white_end - vmin) / (vmax - vmin)

    n_blues = int(n_colors * rel_white_start)
    n_white = int(n_colors * (rel_white_end - rel_white_start))
    n_reds = n_colors - n_blues - n_white

    # Colormaps
    blues = [plt.cm.Blues_r(x) for x in np.linspace(0.3, 1, n_blues)]
    whites = [(1.0, 1.0, 1.0, 1.0)] * n_white
    reds = [plt.cm.Reds(x) for x in np.linspace(0, 1, n_reds)]

    colors = blues + whites + reds
    return mcolors.ListedColormap(colors)


# Example usage
vmin = 1 - 0.6
vmax = np.max(rho_matrix)
white_start = 1.0
white_end = 1.01
...
cmap = create_custom_colormap_with_white_band(vmin, vmax, white_start, white_end)
norm = mcolors.Normalize(vmin=vmin, vmax=vmax)
...
plt.text(ax, ay, s=r'\textbf{b}',fontsize = 10)  # ,bbox={'facecolor':'limegreen','alpha':0.8,'edgecolor':'none','boxstyle':'round,pad=0.25'},
# ha='center', va='center')
plt.text(bx, by, s=r'\textbf{c}',fontsize = 10)  # ,bbox={'facecolor':'limegreen','alpha':0.8,'edgecolor':'none','boxstyle':'round,pad=0.25'},
# ha='center', va='center')
plt.text(cx, cy, s=r'\textbf{d}',fontsize = 10)  # ,bbox={'facecolor':'limegreen','alpha':0.8,'edgecolor':'none','boxstyle':'round,pad=0.25'},
# ha='center', va='center')
plt.text(dx, dy, s=r'\textbf{e}',fontsize = 10)  # ,bbox={'facecolor':'limegreen','alpha':0.8,'edgecolor':'none','boxstyle':'round,pad=0.25'},
# ha='center', va='center')
...
axR[1, 1].text(-0.5, 1.07 - 0.5, s=r'\textbf{(e)}', fontweight='black', fontsize=14,
               bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.'))
axL.text(99, 327, s=r'\textbf{(a)}', fontsize=14,
         bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.'))
