...
mu = [100 + 10 * i for i in range(71)]
Pe = [0 + 5 * i for i in range(65)]
print(Pe)
...
pm = plt.imshow(rho_matrix,cmap="gnuplot",extent=np.concatenate((bounds, bounds2)), origin ='lower',aspect='auto')
...
plt.plot([185.192,760],[0,320],ls = '--', color = 'white',alpha = 0.8)
...
cbar = fig.colorbar(pcm, ax=axR,  ticks=[1.0, 7, 14],format=tkr.FormatStrFormatter('%.1f'))
cbar.ax.tick_params(labelsize=10)
