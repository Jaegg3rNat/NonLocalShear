...
def update_step(u_hat, kx_, ky_, vx, vy, D, m2, g):
    ...
    nonlinear_term = -1 / K * fftpack.fft2(u * fftpack.ifftshift(fftpack.ifft2(u_hat * fftpack.fft2(m2))))

    # Combine terms with additional constant term (if r is defined elsewhere)
    u_hat_updated = diffusion_term + advection_term_x + advection_term_y + r * u_hat + nonlinear_term
...
# Competition radius of the kernel
comp_rad = 0.2
...
mu = float(sys.argv[1])  # Dimensionless birth rate from command-line argument
r = mu * D / comp_rad ** 2  # Birth rate (biological parameter)

pe = float(sys.argv[2])  # Peclet number from command-line argument
gamma = [pe * comp_rad * (D / comp_rad ** 2)]  # List of velocity multipliers
...
________________________________________________________________________________________________________
________________________________________________________________________________________________________
________________________ NON-LOCAL COMPETITION KERNEL SECTION __________________________________________
________________________________________________________________________________________________________
________________________________________________________________________________________________________
"""

# Calculate the radius in grid points corresponding to the competition radius
r_int = int(comp_rad / dx)  # Number of grid points within the competition radius

# Initialize the kernel matrix
num_points_inside = 0
m = np.zeros((1 + 2 * r_int, 1 + 2 * r_int))  # Kernel matrix of appropriate size
m_norm = np.pi * comp_rad ** 2  # Continuum Normalization constant for the 2D kernel
...
main_directory = "simulation_results"
if not os.path.exists(main_directory):
    os.makedirs(main_directory)

# Automatically create subdirectory based on nx value
lattice_size_dir = f"{main_directory}/{nx}_R{comp_rad}"
...
for g in gamma:
    ...
    path = f"{lattice_size_dir}/{flow_type}_mu{mu:.2f}_Pe{Pe:.1f}_w{w:.2f}"
    ...
    for n in tqdm(range(1, nt)):
        ...
        if flow_type == "sinusoidal" and n <= 5000:
            ...
            gamma = g
        # Use if want the flow from beginning
        # gamma = g
        # Compute time step
        u = rk4_pseudospectral(u, vx, vy, gamma, D, m2, dt, n_step=1, kx=kx, ky=ky)
